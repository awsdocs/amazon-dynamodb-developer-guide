# The `DynamoDBMapper` Class<a name="DynamoDBMapper.Methods"></a>

The `DynamoDBMapper` class is the entry point to DynamoDB\. It provides access to a DynamoDB endpoint and enables you to access your data in various tables; perform various create, read, update, and delete (CRUD) operations on items; and execute queries and scans against tables\. This class provides the following methods for working with DynamoDB\.
...
If an item that has the same primary key does not exist, this method creates a new item in the table\. If an item that has the same primary key exists, it updates the existing item\. If the partition key and sort key are of type String, and annotated with `@DynamoDBAutoGeneratedKey`, they are given a random universally unique identifier \(UUID\) if left uninitialized\. Version fields annotated with `@DynamoDBVersionAttribute` will be incremented by one\. Additionally, if a version field is updated or a key generated, the object passed in is updated as a result of the operation\. 
...
Assume that you have created a mapping between a *Reply* class and the corresponding *Reply* table in DynamoDB\. The following Java code example uses `DynamoDBMapper` to find all replies in the past two weeks for a specific thread subject\.
...
Queries a table or secondary index and returns a single page of matching results\. As with the `query` method, you must specify a partition key value and a query filter that is applied on the sort key attribute\. However, `queryPage` will return only the first "page" of data \- that is, the amount of data that will fit within 1 MB 
...
For a Java code example illustrating use of `parallelScan`, see [Example: Query and Scan](DynamoDBMapper.QueryScanExample.md)\.

## `batchSave`<a name="DynamoDBMapper.Methods.batchSave"></a>

Saves objects to one or more tables by using one or more calls to the `AmazonDynamoDB.batchWriteItem` method\. This method does not provide transaction guarantees\.
...
Retrieves multiple items from one or more tables by using their primary keys\.
...
Deletes objects from one or more tables by using one or more calls to the `AmazonDynamoDB.batchWriteItem` method\. This method does not provide transaction guarantees\. 
...
Saves objects to and deletes objects from one or more tables by using one or more calls to the `AmazonDynamoDB.batchWriteItem` method\. This method does not provide transaction guarantees or support versioning \(conditional puts or deletes\)\.
