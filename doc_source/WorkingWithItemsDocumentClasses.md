...
To perform data operations using the document model, you must first call the `Table.LoadTable` method, which creates an instance of the `Table` class that represents a specific table\. The following C\# snippet creates a `Table` object that represents the `ProductCatalog` table in Amazon DynamoDB\.
...
/*Other attributes have been omitted for brevity...*/
